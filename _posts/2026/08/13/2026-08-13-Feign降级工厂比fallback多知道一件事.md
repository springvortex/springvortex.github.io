...
> 2026-08-24 更新：用户接口的降级策略已从“返回空数据”调整为统一返回 `code=503` 的业务繁忙失败响应。本文的重点仍是
> `FallbackFactory` 能拿到失败原因；具体策略以《用户 Feign 降级要承认服务不可用》为准。
...
这适合演示和部分读取场景，但会把“数据不存在”和“下游不可用”混在一起。当前用户接口已改为统一返回业务繁忙失败响应；下单、扣库存、支付这类操作更应该暴露失败，让用户重试，而不是返回一个看似成功的空结果。
