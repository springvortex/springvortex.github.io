...
文章链接由 `_plugins/md5_permalink.rb` 计算，MD5 输入只有三项：发布日期、作者和标题。发布日期会按 UTC+8
时区格式化；文章没有单独声明作者时，使用站点作者 `zjc`。

但要注意：不要手工修改 `_site` 产物，也不要把 `_site` 提交回仓库。部署产物每次都会重新生成，改源文件才是正确入口。

#### 6. 改一个错别字，文章链接会变吗

不会。正文、分类、标签、注释、Front Matter 其他字段和空白字符都不参与 MD5 计算，所以修改文章内容不会让链接变化。

MD5 输入只有发布日期、作者和标题。这三项是永久链接的身份字段；发布后修改其中任意一项，会生成新链接，旧路径不会被自动重定向。

因此发布前只需要确认这三项信息正确。发布后修正错别字、补充示例代码或调整文章结构，都不会影响文章链接。
